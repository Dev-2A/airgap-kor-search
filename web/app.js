...
function renderSearchResults(data) {
  ...
  searchResults.innerHTML = data.results
    .map((r, i) => {
      ...
      // 하이라이트 텍스트가 있으면 사용, 없으면 일반 텍스트
      const hlText = r.highlighted_text || r.text;
      const preview =
        hlText.length > 350 ? hlText.slice(0, 350) + "..." : hlText;
      ...
      return `
                <div class="result-card">
                    ...
                    <div class="result-text">${sanitizeHighlight(preview)}</div>
                </div>
            `;
    })
    .join("");
...
async function deleteDoc(docPath) {
  ...
  try {
    const res = await fetch(
      `${API.documents}?doc_path=${encodeURIComponent(docPath)}`,
      {
        method: "DELETE",
      },
    );
